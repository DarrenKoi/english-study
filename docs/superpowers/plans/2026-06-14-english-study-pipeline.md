...
**Tech Stack:** Python 3.11 (stdlib only: `json`, `subprocess`, `pathlib`, `datetime`, `re`, `fnmatch`), pytest, git, `claude -p --output-format json`, macOS launchd.
...
├─ tests/
│   ├─ conftest.py               # scripts/ 를 sys.path 에 추가
│   └─ test_*.py
...
- [ ] **Step 2: `tests/conftest.py` 작성** (scripts/ 를 import 경로에 추가)

```python
import sys
from pathlib import Path

# scripts/ 를 sys.path 에 넣어 `from pipeline.X import Y` 가 되게 한다
SCRIPTS = Path(__file__).resolve().parents[1] / "scripts"
if str(SCRIPTS) not in sys.path:
    sys.path.insert(0, str(SCRIPTS))
```

- [ ] **Step 3: `config/sources.json` 작성** — 위 표준 스키마 그대로 사용 (3개 repo)
...
- [ ] **Step 6: pytest 가 빈 상태로 동작하는지 확인**

Run: `python -m pytest -q`
Expected: `no tests ran` (collection 에러 없이 종료)

- [ ] **Step 7: Commit**

```bash
git add scripts/pipeline/__init__.py tests/conftest.py config/sources.json .gitignore pytest.ini
git commit -m "chore: scaffold english-study pipeline package and test infra"
```
...
Run: `python -m pytest tests/test_gitutil.py -q`
...
Run: `python -m pytest tests/test_transcripts.py -q`
Expected: FAIL (module/attr 없음)
...
Run: `python -m pytest tests/test_requests_inbox.py -q`
Expected: FAIL
...
Run: `python -m pytest tests/test_tokenlog.py -q`
Expected: FAIL
...
Run: `python -m pytest tests/test_finalize.py -q`
Expected: FAIL
...
python3 -m pipeline.run >> "$ROOT/state/run.log" 2>&1
```
...
## 수동 실행
`PYTHONPATH=scripts python3 -m pipeline.run`
```
...
- [ ] **Step 1: 수동 1회 실행으로 end-to-end 확인**
...
- 수동 실행: `PYTHONPATH=scripts python3 -m pipeline.run`
EOF
git add README.md
