...
# 오늘의 표현 (5차 · 야간) — `repo:auto_recipe_creator` grill-with-docs / 저널

지침서 영어(grill-with-docs)와 한국어 저널에서, 회의·문서에서 바로 쓰는 표현 위주.

1. **sharpen fuzzy language** — 두루뭉술한 말을 표준 용어로 또렷이 다듬다.
2. **call it out / surface it** — 모순을 그 자리에서 짚어 드러내다.
3. **offer (something) sparingly** — 아껴서·드물게만 하라 (↔ liberally).
4. **don't batch these up — capture them as they happen** — 모아두지 말고 즉시 기록하라.
5. **stress-test a plan with specific scenarios** — 구체적 시나리오로 계획을 두드려 약점 찾기.
6. **pin down a faint box / benchmark against real data** — 흐릿한 걸 정확히 짚다 / 실데이터로 성능 재기. (저널 한글→영어)

### 오늘의 정독
- **단락 3**: `reading.md` — "Only offer ... when all three are true" (`Only ... when` 제한 + 형용사구 병렬 체크리스트 문체).
- **단락 4**: `reading.md` — `When ... , [명령형]` 4연속 반복(parallelism) + 현재진행 수동태 `are being discussed`.

> 처리 항목 7개(표현) + 정독 2단락 + 코칭 3장 / 미뤄진 항목 0개 (4·5차로 배치 49개 문서 정독 완료; 나머지는 코드·도메인 고유어)
